...
function changeLineWithEnter(inputHeight, target) {
  ...
  changeCSS(target, "height", inputHeight + 10 + "px");
}

// 사용자의 입력값에 해당하는 높이값으로 설정하는 함수
const resizeTextarea = (object) => pipe(
  () => changeCSS(object, "height", "auto"),
  () => changeCSS(object, "height", object.scrollHeight + "px")
)();

// 사용자가 엔터를 입력하거나 지우면 그에 따라 줄바꿈 이벤트를 할당하는 함수
const resizeTextareaEvent = (target) => pipe(
  ($txtArea) => $txtArea.forEach((txt) => {
    changeCSS(txt, "height", txt.scrollHeight + 16 + "px");
